...
Read from RomM 4.9.2's live `openapi.json` on an LXC container. Use these exactly.
...
```
cd /c/Example/romm-hub && git archive HEAD -o /tmp/hub.tar && scp /tmp/hub.tar root@your-server.example:/tmp/hub.tar && ssh root@your-server.example "pct push <ctid> /tmp/hub.tar /tmp/hub.tar && pct exec <ctid> -- bash -c 'rm -rf /tmp/hub && mkdir -p /tmp/hub && tar xf /tmp/hub.tar -C /tmp/hub'"
```
...
**Then one real end-to-end import**, run manually and recorded in the report: pick a small, freely-downloadable, non-`stream_only` Archive.org item, import it into the live RomM on an LXC container, and confirm it appears in the library. **Ask the operator before writing to the live library** — this is the first write this project has ever made to real data. Report the item chosen and the resulting rom id.
