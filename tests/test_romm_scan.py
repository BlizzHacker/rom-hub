...
from romm_hub.romm.scan import (
    SCAN_DONE_EVENT,
    SCAN_DONE_KO_EVENT,
    SCAN_EVENT,
    SOCKETIO_PATH,
...
class FakeRomm:
    def __init__(self, token="tok-abc", base_url="http://romm.example:8080"):
        self._token = token
...
def test_connect_targets_romms_own_socketio_path_not_the_default():
    client = FakeSocketClient(reply=(SCAN_DONE_EVENT, DONE_STATS))
    _scanner(client, romm=FakeRomm(base_url="http://romm.example:8080")).scan_platform(1)

    url, kwargs = client.connect_calls[0]
    assert url == "http://romm.example:8080"
